...
const Translate_API_KEY = "AIzaSyAyckSRqTR_JGZhRnXoZtgIMvgglpPZt4o"; //process.env.TRANSLATE_API_KEY;
let language = "en";
    ...
    async function onReceiveMessage(event) {
      ...
      if (
        ...
        const response = await fetch(
          `https://translation.googleapis.com/language/translate/v2?key=${Translate_API_KEY}&q=${newMessage.body}&target=${language}`
        );
        const data = await response.json();
        newMessage.body = data.data.translations[0].translatedText;
      }
  ...
  return (
    <ChatSidebar onClose={onClose}>
      ...
      <ChatInput
        ...
        afterInput={
          <>
            {<TranslatePopoverButton onSelectLang={onSelectLang} lang={lang} />}

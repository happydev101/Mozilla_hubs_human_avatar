...
import { CustomLoginModel } from "./custom/CustomLoginModel";
import { SuggestionBoxModel } from "./custom/SuggestionBoxModel";
...
import { ReactComponent as PersonControl } from "./icons/Video.svg";
...
import { LiveCaptionContainer } from "./room/LiveCaptionContainer";
import { LiveCaptionTextContainer } from "./room/LiveCaptionTextContainer";
import { SpeechRecognition } from "./room/SpeechRecognition";
...
import { ExternalSceneManager } from "../customcode/external-scene-manager";
...
class UIRoot extends Component {
  ...
  componentDidMount() {
    ...
    ExternalSceneManager.getInstance().addEventListener("showSuggestionBox", this.showSuggestionBox);
  }
  showSuggestionBox = data => {
  ...
  componentWillUnmount() {
    ...
    ExternalSceneManager.instance.addEventListener("showSuggestionBox", this.showSuggestionBox);
  }
  ...
  toggleCaption() {
    this.setState(prevState => ({
      captionEnable: !prevState.captionEnable
    }));
  }

  toggleAloneState() {
    this.setState(prevState => ({
      aloneState: !prevState.aloneState
    }));
  }

  toggleSpeechRecognition(SRenableState) {
    console.log(SRenableState)
    this.setState({...this.state, speechRecognitionEnable: SRenableState})
  }
  onFocusChat = e => {
    ...
    return (
      <>
        <RoomEntryModal
          ...
          onJoinRoom={() => {
            //prompt for custom login
            this.pushHistoryState("entry_step", "customlogin");
    ...
    const entryDialog =
      ...
      (this.isWaitingForAutoExit() ? (
        ...
        <>
          ...
          <StateRoute stateKey="entry_step" stateValue="customlogin" history={this.props.history}>
            {this.renderCustomLoginPanel()}
          </StateRoute>
          <StateRoute
            stateKey="entry_step"
            stateValue="profile"
          ...
          <div className={classNames(rootStyles)}>
            ...
            {this.props.hub && (
              <RoomLayoutContainer
                ...
                viewport={
                  <>
                    {this.state.showSuggestion ? this.renderSuggestionPanel() : undefined}
                ...
                livecaptiontext={entered && this.state.captionEnable && <LiveCaptionTextContainer />}
                  ...
                  <>
                    ...
                    {entered && (
                      <LiveCaptionContainer
                        ...
                        captionenable={this.state.captionEnable}
                        mySessionId={this.props.sessionId}
                        presences={this.props.presences}
                        onPeopleChange={() => this.toggleAloneState()}
                      />
                    )}
                    {entered && isMobileVR && (
                      <ToolbarButton
                    ...
                    {entered && this.state.aloneState && (
                      <SpeechRecognition 
                        onMouseDown={() => this.toggleSpeechRecognition(true)}
                        onMouseUp={() => this.toggleSpeechRecognition(false)}
                        speechRecognitionEnable={this.state.speechRecognitionEnable}
                      />
                    )}
                  </>
                }

...
import { MyCameraTool, AiAvatar } from "./bit-components";
...
export default class SceneEntryManager {
  ...
  _setupAICharacter = () => {
    // if(this._entered == true)
    this.scene.addEventListener("action_toggle_AI_character", event => {
      const myAIAvatar = anyEntityWith(APP.world, AiAvatar);
      if (event.detail === 1) {
        const avatarPov = document.querySelector("#avatar-pov-node").object3D;
        const eid = createNetworkedEntity(APP.world, "avatar");
        addComponent(APP.world, AiAvatar, eid);

        const obj = APP.world.eid2obj.get(eid);
        obj.position.copy(avatarPov.localToWorld(new THREE.Vector3(0, 0, -2)));
        // obj.rotateY(0.5);
        obj.lookAt(avatarPov.getWorldPosition(new THREE.Vector3()));
      } else {
        console.log("another person entered ^^^^^^^^^^^^", myAIAvatar);
        removeEntity(APP.world, myAIAvatar);
      }
    });
  };
  ...
  _spawnAvatar = () => {
    ...
    ExternalSceneManager.getInstance().load();
  };
